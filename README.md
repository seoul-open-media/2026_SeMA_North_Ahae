...
아래는 현재 펌웨어가 실제로 동작하는 흐름을 코드 기준으로 풀어쓴 설명입니다.

### 1) 시작 단계 (setup)
1. USB 시리얼(115200) 시작 후 타임아웃을 20ms로 짧게 설정합니다.
2. OLED, XBee(Serial4), 전자석/솔레노이드, SPI, CAN 순서로 초기화합니다.
3. Moteus 통신 포맷과 기본 제한값을 준비한 뒤, 두 모터에 `SetStop()`을 보내 초기 fault를 정리합니다.

### 2) 메인 루프 (loop)
루프에서는 다음 작업이 반복됩니다.
1. 50ms 주기(20Hz)로 모터 상태를 쿼리합니다.
2. fault 모드가 감지되면 `SetStop()`을 여러 번 보내 fault를 해제하고 제어 상태를 안전 모드로 내립니다.
3. 모터 온도(약 250ms 주기)를 감시하고 과열 시 전체 정지합니다.
4. OLED를 주기적으로 갱신해 위치/온도/상태를 표시합니다.
5. XBee 패킷(Serial4)을 확인해 목표 각도와 속도 명령을 디코딩합니다.
6. 전자석/솔레노이드 타이머를 업데이트해 펄스 구동을 마무리합니다.

### 3) XBee 명령 처리
1. 헤더 `0xFF 0xFF`를 확인하고 26바이트(13대 로봇 x 2바이트)를 읽습니다.
2. 본인 `ROBOT_ID` 슬롯의 2바이트를 4자리 숫자로 해석합니다.
3. 자리별 의미는 다음과 같습니다.
  - digit1: 속도 스케일
  - digit2: 예약(현재 미사용)
  - digit3: 목표 각도(-90도~+90도)
  - digit4: 작동기 트리거(전자석/솔레노이드)
4. 현재 M1 절대 위치를 읽고, M2 목표 위치와 함께 `moveToEncoderPosition()`을 호출합니다.

### 4) 모터 이동 제어 (moveToEncoderPosition)
1. 100Hz 루프로 동작하며 외부 엔코더(abs_position) 오차를 기준으로 속도를 생성합니다.
2. 목표와 멀면 빠르게, 가까우면 비례 감속하고 허용 오차 이내에서 정지 판정을 수행합니다.
3. 오버슈트(목표를 지나침)와 엔코더 스톨(값 고정) 조건을 감시합니다.
4. 이상 상태면 즉시 `SetStop()`으로 모터를 멈추고 루프를 종료합니다.
5. 지정 시간(TIMEOUT_MS) 내 도달하지 못하면 타임아웃 정지합니다.

### 5) 전자석/솔레노이드 제어
1. 전자석 3개와 솔레노이드는 비차단 타이머 방식으로 구동됩니다.
2. 브레이크 모드에서는 엔코더 변화량을 누적 관찰해 모터가 충분히 안정되면 전자석을 해제합니다.
3. 최소 유지 시간과 최대 타임아웃이 모두 있어, 너무 빨리 풀리거나 무한 유지되는 상황을 방지합니다.

### 6) 설정값 튜닝 포인트
아래 값은 로봇별로 가장 자주 조정됩니다.
1. `ROBOT_ID`: 패킷 내 로봇 슬롯 선택
2. `MOTOR_OFFSET`: 기구 조립 오차 보정
3. `ENCODER_CENTER`: 기계적 0도 기준점
4. `MOTOR*_DIRECTION`: 모터 회전 방향 보정
5. `POSITION_TOLERANCE`, `SLOW_ZONE`, `TIMEOUT_MS`: 응답성과 안정성 균형
